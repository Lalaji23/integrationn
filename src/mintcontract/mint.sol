...
interface IPhantom{
    ...
    function getCreator(uint256 tokenId) external view returns(address);
    function royaltyFee(uint256 tokenId)external view returns(address);

}

contract LogicContract is ERC721{
...
contract Proxy is ERC721 {
    ...
    function createCollectible(string memory tokenURI, uint256 fee) external{
       (bool success,)=logiccontractaddress.delegatecall(
            abi.encodeWithSignature("createCollectible(string,uint256)",tokenURI,fee)
        );
    ...
    function changeLogicContractAddress(address _address) public onlyOwner{
